...
def resource_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            resource.save()
            Activity.objects.create(  user=request.user, action="Resource Created",description=f"Added resource: {resource.title}"  )
...
def resource_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            resource = form.save()
            Activity.objects.create( user=request.user,action="Resource Updated", description=f"Updated resource: {resource.title}" )
...
def resource_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        resource.delete()
        Activity.objects.create( user=request.user, action="Resource Deleted", description=f"Deleted resource: {title}"  )
